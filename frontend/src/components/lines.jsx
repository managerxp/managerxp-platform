...
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  float lineCore = smoothstep(0.013, 0.0, abs(distanceToLine));
  float lineEdge = smoothstep(0.026, 0.0, abs(distanceToLine)) * 0.06;
  float lineGlow = smoothstep(0.085, 0.0, abs(distanceToLine)) * 0.045;
  return lineCore * 1.3 + lineEdge + lineGlow;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableBottom) {
    for (int i = 0; i < bottomLineCount; ++i) {
      ...
      vec3 lineCol = getLineColor(t, BLACK);
  ...
  float vignette = smoothstep(1.65, 0.25, length(baseUv * vec2(0.9, 1.15)));
  col *= vignette;
...
export default function LinesBackground() {
  const lineConfig = {
    linesGradient: ['#b31212', '#d91c1c', '#ff4242'],
    animationSpeed: 0.38,
    interactive: true,
    bendRadius: 7.5,
    bendStrength: -0.55,
    ...
    lineCount: [6, 7, 5],
    lineDistance: [6.2, 5.1, 6.8]
  };

  return (
    <>
      <div className="absolute inset-0 z-0 bg-black">
        <FloatingLines {...lineConfig} />
      </div>
      <div className="pointer-events-none absolute inset-0 z-10 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(0,0,0,0.12)_58%,rgba(0,0,0,0.48)_100%)]" />
      <div
        className="pointer-events-none absolute inset-0 z-10 opacity-5"
        style={{
          background:
            'linear-gradient(to bottom, rgba(255, 0, 0, 0), rgba(255, 0, 0, 0) 50%, rgba(255, 0, 0, 0.06) 50%, rgba(255, 0, 0, 0.06))',
